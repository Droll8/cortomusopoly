...
// === BANDWIDTH SAVER ===
// gzip compression: riduce HTML/JS/JSON di ~70-80% (1.4MB → ~400KB per il file di gioco).
// `threshold: 1024` = comprime solo risposte > 1 KB (evita overhead su risposte minuscole).
// `filter`: non comprime gli audio MP3 (già compressi, gzip li renderebbe più grandi).
app.use(compression({
    threshold: 1024,
    filter: (req, res) => {
...
const io = new Server(server, {
    ...
    // BANDWIDTH SAVER: compressione per Socket.IO (riduce drasticamente le banda usata da state-updated).
    // perMessageDeflate è abilitato di default in Socket.IO v4 ma ottimizziamo le soglie.
    perMessageDeflate: {
        threshold: 1024,        // comprimi messaggi > 1 KB
        zlibDeflateOptions: { level: 6 }, // bilanciato (default 6)
...
// 19.0+: distribuzioni alternative per i bot "favoriti" (più gol fatti, meno gol subiti).
// Scelti a inizio partita (2-3 bot random), nascosti ai giocatori.
function sampleOwnGoalsForFavoredBot() {
    const r = Math.random();
    if (r < 0.15) return 0;
    if (r < 0.45) return 1;
    if (r < 0.75) return 2;
...
function sampleOppGoalsForFavoredBot() {
    // Distribuzione spostata verso 0/1 gol subiti
    const r = Math.random();
    if (r < 0.55) return 0;
    if (r < 0.82) return 1;
    if (r < 0.94) return 2;
...
function generateBotMatchResult(favored) {
    if (favored) {
        return { own: sampleOwnGoalsForFavoredBot(), opp: sampleOppGoalsForFavoredBot() };
    }
...
function simulateBotsMatchday(room) {
    ...
    for (const bot of room.bots) {
        // 19.0+: bot "favoriti" hanno una distribuzione spostata (più gol fatti, meno subiti).
        const r = generateBotMatchResult(!!bot.favored);
        bot.stats.played++;
        bot.stats.gf += r.own;
        bot.stats.gs += r.opp;
        let points;
...
// 19.0+: simula le giornate mancanti per i giocatori che hanno matematicamente vinto il campionato
// e hanno scelto "NO, smetto di giocare". I loro match futuri vengono simulati come per i bot,
// in modo che la classifica resti coerente fino alla 38ª giornata.
function syncChampionStoppedMatchdays(room) {
    if (!room.championStoppedSockets) return;
    const ids = Object.keys(room.championStoppedSockets);
    if (!ids.length) return;
    if (!room.matchHistoryById) room.matchHistoryById = {};
    const cap = Math.min(MAX_MATCHDAYS, maxConnectedUserPlayed(room));
    if (cap <= 0) return;
    ids.forEach(socketId => {
        if (!room.championStoppedSockets[socketId]) return;
        ensureUserStats(room, socketId);
        const s = room.standings[socketId];
        if (!s) return;
        if (!room.matchHistoryById[socketId]) room.matchHistoryById[socketId] = [];
        ...
        while ((s.played || 0) < cap && (s.played || 0) < MAX_MATCHDAYS) {
            ...
            room.matchHistoryById[socketId].push({
                own: r.own, opp: r.opp, points,
                verdict: points === 3 ? 'W' : (points === 1 ? 'D' : 'L'),
                oppName: '— (simulato: campione fermo)',
                matchday: s.played
            });
            simulated++;
        }
        if (simulated > 0) {
            const name = room.playerNamesById[socketId] || socketId.slice(0, 6);
            console.log(`[sim:champion-stopped] ${name} → simulate ${simulated} giornate (totale ${s.played}/${MAX_MATCHDAYS})`);
        }
    });
}

// 19.0+: allinea un NUOVO giocatore (non disconnesso) entrato a partita in corso al livello
// attuale della stanza, simulando le sue giornate mancanti con risultati random — come per i bot.
// Così quando entra, il suo standings.played è già pari al numero medio di giornate giocate dagli altri.
function syncSingleNewJoinerMatchdays(room, socketId) {
    if (!room.gameStarted) return;
    ensureUserStats(room, socketId);
    const s = room.standings[socketId];
    if (!s) return;
    if ((s.played || 0) > 0) return; // non è un new joiner — ha già giocato
    const cap = Math.min(MAX_MATCHDAYS, maxConnectedUserPlayed(room));
    if (cap <= 0) return;
    if (!room.matchHistoryById) room.matchHistoryById = {};
    if (!room.matchHistoryById[socketId]) room.matchHistoryById[socketId] = [];
    ...
    while ((s.played || 0) < cap && (s.played || 0) < MAX_MATCHDAYS) {
        ...
        room.matchHistoryById[socketId].push({
            own: r.own, opp: r.opp, points,
            verdict: points === 3 ? 'W' : (points === 1 ? 'D' : 'L'),
            oppName: '— (simulato: ingresso a partita in corso)',
            matchday: s.played
        });
        simulated++;
    }
    if (simulated > 0) {
        const name = room.playerNamesById[socketId] || socketId.slice(0, 6);
        console.log(`[sim:new-joiner] ${name} → simulate ${simulated} giornate per allinearlo al livello attuale (played=${s.played}/${MAX_MATCHDAYS})`);
    }
}
...
function getRoom(roomId) {
    if (!rooms[roomId]) {
        rooms[roomId] = {
            ...
            marketMode: 'off',            // 19.0: 'off' (default) | 'on' — sistema mercato/sponsor
            ...
            economy: {},
            // === 19.0+: socketId → { stoppedAt: timestamp } — giocatori che hanno matematicamente vinto
            // il campionato E hanno scelto di NON continuare a giocare. I loro turni vengono saltati e
            // le giornate rimanenti vengono simulate come per i bot/disconnessi.
            championStoppedSockets: {}
        };
    }
    return rooms[roomId];
}
...
function snapshot(room) {
    ...
    return {
        ...
        marketMode: room.marketMode || 'off',
        ...
        // === 19.0+: lista socket che hanno scelto di smettere dopo vittoria matematica ===
        championStoppedSocketIds: Object.keys(room.championStoppedSockets || {})
    };
}
...
function migratePlayerState(room, oldSocketId, newSocketId) {
    ...
    // 19.0+: Migrate stato "campione-fermo" se presente (chi ha smesso di giocare dopo vittoria matematica)
    if (room.championStoppedSockets && room.championStoppedSockets[oldSocketId]) {
        room.championStoppedSockets[newSocketId] = room.championStoppedSockets[oldSocketId];
        delete room.championStoppedSockets[oldSocketId];
    }
}
...
io.on('connection', (socket) => {
    ...
    socket.on('join-room', (payload = {}) => {
        ...
        if (!reconnected) {
            ...
            // 19.0+: NUOVO GIOCATORE entra a partita in corso → simulo le sue giornate per allinearlo
            // al livello attuale (come per i disconnessi). Solo se gameStarted=true e ha played=0.
            if (room.gameStarted) {
                try { syncSingleNewJoinerMatchdays(room, socket.id); } catch (e) { console.warn('sync new joiner', e); }
            }
        }
    ...
    // 19.0: modalità mercato (sponsor + cassa + pacchetti). Solo host, solo pre-partita.
    socket.on('update-market-mode', (payload = {}) => {
        const roomId = sanitizeRoomId(payload.roomId) || joinedRoomId;
        if (!roomId || !rooms[roomId]) return;
        const room = rooms[roomId];
        if (socket.id !== room.hostSocketId) {
            console.log(`[${roomId}] update-market-mode rifiutato: non sei l'host`);
            return;
        }
        if (room.gameStarted) {
            console.log(`[${roomId}] update-market-mode rifiutato: partita già iniziata`);
            return;
        }
        const mode = (payload.mode === 'on') ? 'on' : 'off';
        room.marketMode = mode;
        console.log(`[${roomId}] marketMode → ${mode}`);
        io.to(roomId).emit('state-updated', snapshot(room));
    });
        ...
        if (payload.inMatch === true) {
            ...
            if (!room.gameStarted) {
                ...
                if (room.gameMode === 'withBots') {
                    ...
                    // 19.0+: a inizio partita scelgo 2-3 bot "favoriti" che avranno una distribuzione
                    // gol migliore (vincite più frequenti) per tutta la stagione. Nascosti ai giocatori.
                    try {
                        const eligibleBots = (room.bots || []).filter(b => b && !b.favored);
                        const favorCount = Math.min(eligibleBots.length, 2 + Math.floor(Math.random() * 2)); // 2 o 3
                        const shuffled = [...eligibleBots].sort(() => Math.random() - 0.5);
                        const picked = shuffled.slice(0, favorCount);
                        picked.forEach(b => { b.favored = true; });
                        console.log(`[${roomId}] bot favoriti (${picked.length}): ${picked.map(b => b.name).join(', ')}`);
                    } catch (e) { console.warn('bot favorites picking', e); }
                } else {
            ...
            const isDisconnected = (id) => (
                (!!(room.disconnectedSockets && room.disconnectedSockets[id])) ||
                (!!(room.championStoppedSockets && room.championStoppedSockets[id]))
            );
    ...
    socket.on('sync-personal-state', (payload = {}) => {
        ...
        const state = {
            ...
            // 19.0+ BUG FIX: NON sovrascrivere lastOpponentId con null se il client non lo manda.
            // Il server è autoritativo per questo campo (lo setta in match-phase-update). Se il client
            // manda null/missing per qualsiasi motivo, preserviamo il valore già presente — così l'esclusione
            // dell'ultimo avversario funziona correttamente match dopo match.
            lastOpponentId: (typeof payload.lastOpponentId === 'string' && payload.lastOpponentId)
                ? payload.lastOpponentId.slice(0, 64)
                : ((room.personalStates[socket.id] && room.personalStates[socket.id].lastOpponentId) || null),
    ...
    socket.on('match-result', (payload = {}) => {
        ...
        // === 19.0+: campioni-fermi simulano anche loro le giornate per restare allineati ===
        try { syncChampionStoppedMatchdays(room); } catch (e) {}
        io.to(roomId).emit('state-updated', snapshot(room));
    ...
    // ====================================================================
    // 19.0+ — CAMPIONE MATEMATICO: decisione "continua o smetti"
    // ====================================================================
    // Quando un giocatore raggiunge la vittoria matematica del campionato, il client mostra un modal
    // e qui riceviamo la sua scelta. Se 'stop' → segnaliamo il socket come fermo, salta i turni
    // e le sue future giornate vengono simulate come per i bot.
    socket.on('champion-decision', (payload = {}) => {
        const roomId = sanitizeRoomId(payload.roomId) || joinedRoomId;
        if (!roomId || !rooms[roomId]) return;
        const room = rooms[roomId];
        const choice = payload.choice === 'stop' ? 'stop' : 'continue';
        if (choice === 'stop') {
            if (!room.championStoppedSockets) room.championStoppedSockets = {};
            room.championStoppedSockets[socket.id] = { stoppedAt: Date.now() };
            const name = room.playerNamesById[socket.id] || socket.id.slice(0, 6);
            console.log(`[${roomId}] 🏆 ${name} ha vinto matematicamente e ha scelto di SMETTERE — turni saltati, match simulati.`);
            // Se era il suo turno, salto subito al prossimo
            try {
                ...
                if (wasCurrentTurn) {
                    ...
                    for (let i = 0; i < N; i++) {
                        ...
                        if (!isFinished(candidate) && !isSkipped(candidate)) break;
                    }
                    room.turnIndex = nextIdx;
                    io.to(roomId).emit('turn-updated', {
                        ...snapshot(room),
                        statusText: `🏆 ${name} ha vinto il campionato e si ritira — il turno passa.`
                    });
                }
            } catch (e) { console.warn('champion-stop turn skip', e); }
            // Simulo subito le giornate residue per allinearmi al cap attuale
            try { syncChampionStoppedMatchdays(room); } catch (e) {}
            io.to(roomId).emit('state-updated', snapshot(room));
        } else {
            console.log(`[${roomId}] ${room.playerNamesById[socket.id] || socket.id.slice(0,6)} ha scelto di CONTINUARE dopo vittoria matematica.`);
            // Nessuna modifica server-side per "continue": il client gestisce il flag locale per non rimostrarlo.
        }
    });
    ...
    socket.on('sponsor-swap-start', (payload = {}) => {
        ...
        // 19.0+: ESCLUDO gli sponsor già posseduti negli ALTRI slot (e anche quello del slot corrente,
        // così l'utente è davvero forzato a cambiarlo). Niente duplicati nella pesca random.
        const excludeIds = (e.sponsors || []).filter(Boolean).map(s => s.sponsorId);
        e.pendingSetup = {
            slotIdx,
            options: drawSponsorOptions(excludeIds),
            isSwap: true
        };
        socket.emit('sponsor-setup-state', {
            done: false,
    ...
    socket.on('pack-buy', (payload = {}) => {
        ...
        // 19.0: l'acquisto è ammesso SOLO durante il proprio turno (server-side guard).
        const currentTurnSid = room.turnOrder && room.turnOrder[room.turnIndex];
        if (currentTurnSid !== socket.id) {
            socket.emit('pack-denied', { reason: 'not_your_turn' });
            return;
        }
        // 19.0: l'acquisto NON è ammesso durante la match phase (giorno della partita).
        if (room.matchPhaseIds && room.matchPhaseIds.has(socket.id)) {
            socket.emit('pack-denied', { reason: 'in_match_phase' });
            return;
        }
        if (e.cash < price) {
    ...
    socket.on('load-save', (payload = {}) => {
        ...
        try {
            ...
            room.marketMode = (state.marketMode === 'on') ? 'on' : 'off';
    ...
    socket.on('reassign-opponent', (payload = {}) => {
        ...
        // Esclude me, l'attuale opp e i disconnessi / campioni-fermi
