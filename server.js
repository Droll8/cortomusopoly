...
const RECONNECT_GRACE_MS = 60 * 1000; // 60 secondi
...
io.on('connection', (socket) => {
    ...
    socket.on('match-phase-update', (payload = {}) => {
        ...
        if (payload.inMatch === true) {
            ...
            // Assegna avversario casuale (escluso me stesso E l'ultimo affrontato, se possibile)
            const allCandidates = Object.keys(room.playerNamesById).filter(id => id !== socket.id);
    ...
    socket.on('end-turn', (payload = {}) => {
        ...
        if (room.turnOrder.length > 0) {
            ...
            // Usa standings[id].played come fonte autoritativa server-side.
            const isFinished = (id) => {
            ...
            // Avanza l'indice almeno una volta, poi salta i giocatori finiti.
            // Hard cap di sicurezza (turnOrder.length iterazioni) per evitare loop infiniti
            // quando TUTTI hanno finito (in tal caso si ferma sul prossimo qualunque sia).
    ...
    socket.on('disconnect', () => {
        ...
        // Notifica gli altri della "presence" attuale (utente ancora elencato finché grace attivo)
        // ma marchio il giocatore come "fantasma" rimovendo solo i flag attivi del turno
        const wasCurrentTurn = room.turnOrder[room.turnIndex] === capturedSocketId;
        // Se era il suo turno e si disconnette, il turno passa subito (così non blocca gli altri)
        if (wasCurrentTurn) {
            const idx = room.turnOrder.indexOf(capturedSocketId);
            if (idx >= 0) {
                room.turnIndex = (idx + 1) % Math.max(1, room.turnOrder.length);
            }
